...
from api.apps.exhibitions.graphql.types import (
...
class ExhibitionServiceGraphQL:
    ...
    async def extract_all_data(exhibition: Exhibition) -> dict:
        ...
        exhibition_geo_full = (
            await ExhibitionGeo.select()
            .where(ExhibitionGeo.id == exhibition['exhibition']['geo'])
            .first()
        )
        exhibition_geo = {
        ...
        exhibition_details_full = (
            await ExhibitionDetail.select()
            .where(ExhibitionDetail.id == exhibition['exhibition']['details'])
            .first()
        )
        exhibition_details = {
    ...
    async def get_by_slug(slug: str):
        ...
        exhibition_full = await ExhibitionServiceGraphQL.extract_all_data_graphql(
